...
if errorlevel 1 (
    ...
    podman exec containers_vault_1 vault operator init -key-shares=3 -key-threshold=2 > containers\vault\vault_init_keys.txt
    echo Vault initialized. Unseal keys and root token saved to containers\vault\vault_init_keys.txt
) else (
    echo Vault is already initialized.
)

REM Step 3: Unseal Vault with saved keys (first-time unseal)
for /F "tokens=5" %%i in ('findstr /c:"Unseal Key" containers\vault\vault_init_keys.txt') do (
